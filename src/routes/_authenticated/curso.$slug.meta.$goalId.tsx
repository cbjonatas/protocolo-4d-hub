...
import { ArrowLeft, CheckCircle2, Target, Lock, Loader2, RotateCcw, Trophy, XCircle } from "lucide-react";
...
type Question = {
  ...
  explanation: string;
  order_index: number;
  options: { id: string; label: string; content: string; is_correct: boolean; order_index: number }[];
};

async function fetchGoalQuestions(goalId: string): Promise<Question[]> {
  const { data: qs } = await supabase
    .from("questions")
    .select("id, statement, explanation, order_index, is_published, question_options(id, label, content, is_correct, order_index)")
    .eq("goal_id", goalId)
  ...
  return (qs ?? []).map((q: any) => ({
    ...
    options: (q.question_options ?? []).slice().sort((a: any, b: any) => a.order_index - b.order_index),
  }));
}
...
function GoalPage() {
  ...
  const complete = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["course-full", slug] }); },
  });
  ...
  const submitAttempt = useMutation({
    mutationFn: async () => {
      ...
      const rows = questions.map((q) => {
        ...
        return { question_id: q.id, selected_option_id: selected ?? null, is_correct: !!opt?.is_correct };
      });
      const correct = rows.filter((r) => r.is_correct).length;

      const { data: att, error: attErr } = await supabase
        .from("question_attempts")
        .insert({ user_id: u.user.id, goal_id: goalId, total: questions.length, correct_count: correct, finished_at: new Date().toISOString() })
        .select("id")
        .single();
      if (attErr || !att) throw new Error(attErr?.message);

      await supabase.from("question_answers").insert(rows.map((r) => ({ ...r, attempt_id: att.id })));
      ...
      <div className="mt-4 rounded-2xl border border-border bg-card p-6 shadow-elegant md:p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {(goal as any).subject && <div className="text-sm text-muted-foreground">Assunto: {(goal as any).subject}</div>}
          </div>
        </div>
          ...
          <div className="mt-6">
            {qLoading ? (
              ...
              <div className="rounded-xl border border-border bg-background/60 p-6 text-center">
                <p className="text-muted-foreground">As questões desta meta ainda não foram publicadas.</p>
                {goal.external_url && (
                  <Button asChild className="mt-4" variant="outline">
                    <a href={goal.external_url} target="_blank" rel="noreferrer">Acessar link externo</a>
                  </Button>
                )}
              </div>
            )}
        ...
        {hasQuestions && !finished && (
          <div className="mt-6">
            ...
            <div className="mt-6 rounded-xl border border-border bg-background/60 p-5">
              <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Questão {current + 1}</div>
              <p className="mt-2 whitespace-pre-wrap font-medium">{questions![current].statement}</p>
              <div className="mt-4 space-y-2">
                {questions![current].options.map((o) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${selected ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{o.label}</span>
            ...
            <div className="mt-6 flex justify-between">
              ...
              {current < questions!.length - 1 ? (
                ...
                <Button
                  className="bg-gradient-primary shadow-glow"
                  disabled={submitAttempt.isPending || Object.keys(answers).length < questions!.length}
                  onClick={() => submitAttempt.mutate()}
                >
                  {submitAttempt.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Finalizar"}
                </Button>
              )}
            </div>
          </div>
        ...
        {finished && questions && (
          <ResultView
            ...
            onRestart={() => { setAnswers({}); setCurrent(0); setFinished(null); }}
          />
        )}

        {attempts && attempts.length > 0 && !finished && hasQuestions && (
          <div className="mt-8">
            <h3 className="font-display text-sm font-semibold uppercase tracking-widest text-muted-foreground">Histórico</h3>
            <ul className="mt-3 space-y-2">
              {attempts.slice(0, 5).map((a: any) => (
                <li key={a.id} className="flex items-center justify-between rounded-lg border border-border bg-background/60 p-3 text-sm">
                  <span>{new Date(a.created_at).toLocaleString("pt-BR")}</span>
                  <span className="font-semibold">{a.correct_count}/{a.total} ({Math.round((a.correct_count / Math.max(1, a.total)) * 100)}%)</span>
                </li>
              ))}
            </ul>
          </div>
...
function ResultView({ questions, answers, correct, total, onRestart }: { questions: Question[]; answers: Record<string, string>; correct: number; total: number; onRestart: () => void }) {
    ...
    <div className="mt-6">
      <div className="rounded-2xl border border-border bg-gradient-hero p-6 text-center">
        ...
        <div className="mt-2 flex justify-center gap-6 text-sm">
          <div><span className="text-emerald-400 font-bold">{correct}</span> acertos</div>
          <div><span className="text-rose-400 font-bold">{total - correct}</span> erros</div>
          <div><span className="text-gold font-bold">{pct}%</span> aproveitamento</div>
        </div>
        <Button className="mt-4" variant="outline" onClick={onRestart}><RotateCcw className="mr-1 h-4 w-4" /> Refazer</Button>
      </div>
      ...
      <div className="mt-6 space-y-4">
        {questions.map((q, i) => {
          ...
          return (
            <div key={q.id} className={`rounded-xl border p-4 ${isCorrect ? "border-emerald-500/40 bg-emerald-500/5" : "border-rose-500/40 bg-rose-500/5"}`}>
              ...
              <div className="mt-3 space-y-1.5">
                {q.options.map((o) => {
                  ...
                  return (
                    <div key={o.id} className={`flex items-start gap-2 rounded-md px-3 py-2 text-sm ${
                      isRight ? "bg-emerald-500/15 text-emerald-100" : isSel ? "bg-rose-500/15 text-rose-100" : "bg-background/40 text-muted-foreground"
                    }`}>
                      ...
                      {!isRight && isSel && <span className="ml-auto text-xs font-semibold">Sua resposta</span>}
                    </div>
                  );
                })}
                {!selected && correctOpt && <div className="text-xs text-muted-foreground">Você não respondeu.</div>}
              </div>
              {q.explanation && <div className="mt-3 rounded-md border border-border bg-background/40 p-3 text-xs text-muted-foreground"><strong className="text-foreground">Comentário: </strong>{q.explanation}</div>}
            </div>
          );
        })}
      </div>
